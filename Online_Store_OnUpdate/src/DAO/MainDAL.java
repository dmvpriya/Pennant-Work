...
public class MainDAL implements DAO {
	...
	public List<Product> getAllProducts() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				Product product = new Product(prodId, prodName, prodPrice, prodHsnCode, prodImage, prodCategoryId,
						prodDiscount);
	...
	@Override
	public int getDiscountCouponValue(String disCoupon, float totalAmount) throws SQLException {
		// TODO Auto-generated method stub
		...
		try {
			...
			statement.setString(1, disCoupon);
			statement.setFloat(2, totalAmount);
			statement.execute();
			ResultSet rs = statement.getResultSet();
			while (rs.next()) {
				disValue = rs.getInt(1);
				int discountCouponNoc = getDiscountCouponNoc(disCoupon, totalAmount);
				if (discountCouponNoc > 0) {
					updateDiscountCouponNoc(disCoupon);
				}
			}
	...
	public int getDiscountCouponNoc(String disCoupon, float totalAmount) throws SQLException {
		// TODO Auto-generated method stub
		int disCouponNoc = 0;
		try {
			DBConnection db = new DBConnection();
			Connection con = db.loadProperties();
			CallableStatement statement = con.prepareCall("{call getDisCoupNoc(?,?)}");
			statement.setString(1, disCoupon);
			statement.setFloat(2, totalAmount);
			statement.execute();
			ResultSet rs = statement.getResultSet();
			while (rs.next()) {
				disCouponNoc = rs.getInt(1);
			}
	...
	public int updateDiscountCouponNoc(String disCoupon) throws SQLException {
		// TODO Auto-generated method stub
		int disCouponNoc = 0;
		try {
			DBConnection db = new DBConnection();
			Connection con = db.loadProperties();
			CallableStatement statement = con.prepareCall("{select updateDisCoupNoc(?)}");
			statement.setString(1, disCoupon);
			statement.execute();
