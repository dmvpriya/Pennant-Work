...
$(document).ready(function(){
    ...
    function displayCart() {
        ...
        if (cart && cart.length > 0) {
            ...
            cartDiv.append('<button id="placeOrderButton" background-color="#a6c7a7">Place Order</button>');
        } else {
   ...
   $("#frame3").on("click", "#remove", function() {
    ...
    function placeOrderPage() {
    	...
    	if (cart && cart.length > 0) {
        	...
        	cart.forEach(function(item) {
            	...
            	var productActualPrice = parseFloat(item.price);
            	...
            	var productQuantity = parseInt(item.quantity);
           		...
           		prod_price_with_discount = productActualPrice - (productActualPrice*(productDiscount/100));
           		
           		prod_price_with_discount_gst = (productActualPrice * productGst)/prod_price_with_discount;
                    	...
                    	$.ajax({                    		
        	                ...
        	                success: function(response) {
        	                	...
        	                	prod_price_with_discount = productActualPrice - (productActualPrice*(productDiscount/100));
                           		...
                           		prod_gst_price_with_discount_ = (productActualPrice * productGst)/prod_price_with_discount;
                           		
                           		product_Total_Price_with_discount_gst = (prod_price_with_discount + prod_gst_price_with_discount_)* productQuantity;
                           		
                           		console.log("product_Total_Price_with_discount_gst "+product_Total_Price_with_discount_gst);
        	                	...
        	                	orderDetailsHTML += '<tr style="padding:10px;"><td style="padding:10px;">' + productId + '</td><td style="padding:10px;">' + productName + '</td><td style="padding:10px;">' + productActualPrice + '</td><td style="padding:10px;">' + productGst + '</td><td style="padding:10px;">' + productTotalprice + '</td><td style="padding:10px;">' + productHsnCode + '</td><td style="padding:10px;">' + productQuantity +'</td><td style="padding:10px;">' + productDiscount + '</td><td style="padding:10px;">' + product_Total_Price_with_discount_gst.toFixed(2) + '</td><td style="padding:10px;">' + shippingCharges.toFixed(2) + '</td><td style="padding:10px;">' + shippingChargesGst.toFixed(2) + '</td></tr>';
        	                	$.get("OrderDetailsData.html", function(template) {
        	                        ...
        	                        .replace("{{total_amount}}", totalAmount+shippingChargesTotalGst);
        	                    	cartDiv.append(orderDetailsHTMLFinal);
        	                    	t=parseInt(totalAmount);
        	                    	t=t*100;
        	                    	amount=totalAmount+shippingChargesTotalGst;
        	                	});
        	                	i++;
        	...
        	<%--cartDiv.append('<p style="background-color:#a6c7a7;padding:20px;"><form><label id="ApplyCouplabel">Apply Coupon</label><input type="text" id="couponCode"/><button id="applyButton">Apply</button></form></p>');--%>
    ...
    $("#frame3").on("click", "#applyButton", function() {
        var couponCode = $("#couponCode").val();
        console.log("hhh"+couponCode);
            ...
            success: function(response) {
                ...
                if (cart && cart.length > 0) {
                    ...
                    cart.forEach(function(item) {
                        ...
                        var prod_price_with_discount = productActualPrice - (productActualPrice * (productDiscount / 100));
                        var prod_price_with_discount_gst = (productActualPrice * productGst) / prod_price_with_discount;
                        var product_Total_Price_with_discount_gst = (prod_price_with_discount + prod_price_with_discount_gst) * productQuantity;
                        
                        prod_price += product_Total_Price_with_discount_gst;
                    });
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        success: function(response) {
                            var shippingChargesTotal = response;
                            var totalAmountWithShipping = prod_price + shippingChargesTotal;

                            var newTotalAmount = totalAmountWithShipping - (totalAmountWithShipping * (discountValue / 100));
                            var amountSaved = totalAmountWithShipping - newTotalAmount;
                            ...
                            cart.forEach(function(item) {
                                ...
                                var prod_price_with_discount = productActualPrice - (productActualPrice * (productDiscount / 100));
                                var prod_gst_price_with_discount_ = (productActualPrice * productGst) / prod_price_with_discount;
                                var product_Total_Price_with_discount_gst = (prod_price_with_discount + prod_gst_price_with_discount_) * productQuantity;
                                var shippingCharges = (product_Total_Price_with_discount_gst / prod_price) * shippingChargesTotal;
                                ...
                                $.ajax({
                                    ...
                                    dataType: "json",
                                    success: function(response) {
                                        var ProductGST = response;
                                        var shippingChargesGst = shippingCharges * (ProductGST / 100);
                                        var shippingChargesTotalGst = shippingChargesTotal + shippingChargesGst;

                                        orderDetailsHTML += `
                                            <tr style="padding:10px;">
                                                <td style="padding:10px;">${item.id}</td>
                                                <td style="padding:10px;">${item.name}</td>
                                                <td style="padding:10px;">${productActualPrice}</td>
                                                <td style="padding:10px;">${productGst}</td>
                                                <td style="padding:10px;">${item.totPrice}</td>
                                                <td style="padding:10px;">${productHsnCode}</td>
                                                <td style="padding:10px;">${productQuantity}</td>
                                                <td style="padding:10px;">${productDiscount}</td>
                                                <td style="padding:10px;">${product_Total_Price_with_discount_gst.toFixed(2)}</td>
                                                <td style="padding:10px;">${shippingCharges.toFixed(2)}</td>
                                                <td style="padding:10px;">${shippingChargesGst.toFixed(2)}</td>
                                            </tr>`;

                                        if (orderDetailsHTML.indexOf("{{shippingCharges}}") === -1) {
                                            $.get("OrderDetailsData.html", function(template) {
                                                var orderDetailsHTMLFinal = template
                                                    .replace("{{shippingCharges}}", shippingChargesTotalGst)
                                                    .replace("{{pincode}}", pincode)
                                                    .replace("{{order_details}}", orderDetailsHTML)
                                                    .replace("{{total_amount}}", newTotalAmount.toFixed(2));
                                                    
                                                cartDiv.append(orderDetailsHTMLFinal);
                                                
                                                alert("Coupon applied! You have saved " + amountSaved.toFixed(2) + " units.");
                                            });
                                        }
                                    },
                                    error: function(response) {
