...
$(document).ready(function(){
    ...
    function displayProducts(products) {
        ...
        products.forEach(function(product) {
            ...
            var productHTML = '<div class="Product" style="width:80%;height:50%;margin-top:10px;margin-bottom:10px;margin-left:150px;display: flex;flex-direction: row;align-items: center;justify-content: center;text-align: center;padding: 10px;border: 1px solid #ccc;border-radius: 5px;background-color: #a6c7a7;overflow:auto">' +
                                ...
                                '<div class="Desc" style="width: 100%;">' +
                                    '<table>' +
                                        '<thead>' +
                                            ...
                                            '</tr>' +
                                            '<tr>' +
                                            '<th style="width:200px;height:10px;padding-left:40px" id="ProdGST">' + product.prod_gst + '</th>' +
                                        	'</tr>' +
                                        '</thead>' +
                                    '</table>' +
                                '</div>' +
                                ...
                                '</div>' +
                            '</div>';
            product_price.push(product.prod_price);
            ...
            cart.forEach(function(item) {
                ...
                var cartItemHTML = '<div class="CartItem" style="background-color:#a6c7a7; padding:10px;width:200px;height:300px;margin-left:590px;margin-right:600px;">' +
                                        ...
                                        '<div class="CartItemInfo" style="background-color:white;height:200px;">' +
            									...
            									'<label class="CartItemPrice" style="padding:20px;">ProdPrice:' + item.price + '</label><br>' +
            									'<label class="CartItemGST" style="padding:10px;">ProdGST:' + item.gst + '</label><br>' +
            									'<label class="CartItemGST" style="padding:10px;">ProdPincode:' + item.pincode + '</label><br>' +
            									
            									'<button id="remove" style="padding:10px;">-</button>' +
            								    '<span class="quantity" style="padding:10px;">' + item.quantity + '</span>' +
   ...
   $("#frame3").on("click", "#remove", function() {
    ...
    if (cart && cart.length > 0) {
        var totalAmount = 0;
        var i = 0;
        var orderDetailsHTML = '<div id="checkOutContainer" style="background-color:#a6c7a7;marign:100px;display: flex;justify-content: center;align-items: center;"><table style="border:none;margin:30px;"><tr style="padding:10px;"><th style="padding:10px;">Product Name</th><th style="padding:10px;">Price</th><th style="padding:10px;">Quantity</th><th style="padding:10px;">GST</th><th style="padding:10px;">Pincode</th><th style="padding:10px;">Shipping Charges</th></tr>';
        cart.forEach(function(item) {
            ...
            var shippingCharges = [200, 100, 200];
        ...
        orderDetailsHTML += '<p style="background-color:#a6c7a7;padding:20px;">Total Amount: ' + totalAmount + '</p>';
	...
	$("#frame3").on("click","#checkoutButton",function(){
		window.location.href="https://razorpay.com/payment-link/plink_O7ag0WndpgSkOE/test";
	})
